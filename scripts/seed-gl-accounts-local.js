/**
 * Seed GL Categories and Accounts for ALL jobs in LOCAL MongoDB database
 * This ensures every job has access to the chart of accounts in local dev
 */
...
async function seedGLAccountsForLocal() {
  try {
    // Use LOCAL MongoDB URI - prioritize MONGODB_LOCAL_URI
    const dbUri = process.env.MONGODB_LOCAL_URI;
    if (!dbUri) {
      throw new Error('MONGODB_LOCAL_URI must be set in .env.local for local development');
    }
    ...
    console.log('\n✅ GL Accounts seeded successfully in LOCAL database!');
  } catch (error) {
